movies = [
{
"name": "Usual Suspects", 
"imdb": 7.0,
"category": "Thriller"
},
{
"name": "Hitman",
"imdb": 6.3,
"category": "Action"
},
{
"name": "Dark Knight",
"imdb": 9.0,
"category": "Adventure"
},
{
"name": "The Help",
"imdb": 8.0,
"category": "Drama"
},
{
"name": "The Choice",
"imdb": 6.2,
"category": "Romance"
},
{
"name": "Colonia",
"imdb": 7.4,
"category": "Romance"
},
{
"name": "Love",
"imdb": 6.0,
"category": "Romance"
},
{
"name": "Bride Wars",
"imdb": 5.4,
"category": "Romance"
},
{
"name": "AlphaJet",
"imdb": 3.2,
"category": "War"
},
{
"name": "Ringing Crime",
"imdb": 4.0,
"category": "Crime"
},
{
"name": "Joking muck",
"imdb": 7.2,
"category": "Comedy"
},
{
"name": "What is the name",
"imdb": 9.2,
"category": "Suspense"
},
{
"name": "Detective",
"imdb": 7.0,
"category": "Suspense"
},
{
"name": "Exam",
"imdb": 4.2,
"category": "Thriller"
},
{
"name": "We Two",
"imdb": 7.2,
"category": "Romance"
}
]
#task1
def is_good(movie_name, movie_list):
    for movie in movie_list:
        if movie["name"] == movie_name:
            return movie["imdb"] > 5.5
    return None  
print(is_good("We Two", movies))  
print(is_good("Exam", movies))  
#task 2 
def good_movies(movie_list):
    return [movie for movie in movie_list if movie["imdb"] > 5.5]
print(good_movies(movies))
#task 3
def movies_by_category(movie_list, category_name):
    return [movie["name"] for movie in movie_list if movie["category"] == category_name]
print(movies_by_category(movies, "Romance"))
#task 4
def avg_imdb(movie_list):
    total=sum(movie["imdb"] for movie in movie_list)
    return total / len(movie_list)
print(avg_imdb(movies))
#task 5
def avg_by_category(movie_list, category_name):
    category_movies = [m["imdb"] for m in movie_list if m["category"] == category_name]
    return sum(category_movies) / len(category_movies)
print(avg_by_category(movies, "Crime"))